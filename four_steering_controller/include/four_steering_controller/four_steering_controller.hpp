...
static constexpr size_t NR_REF_ITFS = 4;
...
public:
  ...
  FOUR_STEERING_CONTROLLER__VISIBILITY_PUBLIC controller_interface::CallbackReturn
  configure_odometry() override;

  FOUR_STEERING_CONTROLLER__VISIBILITY_PUBLIC bool update_odometry(
    const rclcpp::Duration & period) override;

  FOUR_STEERING_CONTROLLER__VISIBILITY_PUBLIC void
  initialize_implementation_parameter_listener() override;

protected:
  std::shared_ptr<four_steering_controller::ParamListener> ackermann_param_listener_;
  four_steering_controller::Params ackermann_params_;
};
}  // namespace four_steering_controller
